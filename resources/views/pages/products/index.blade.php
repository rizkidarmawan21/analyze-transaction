                    ...
                    <tbody>
                        @foreach ($products as $item)
                            <tr class="bg-slate-100/25">
                                ...
                                <td class="border border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p class="{{ $item->is_show ? 'text-green-500' : 'text-red-500' }} dark:text-white">
                                        {{ $item->is_show ? 'Yes' : 'No' }}
                                    </p>
                                </td>
                                <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <div class="flex items-center space-x-3.5">
                                        <button type="submit" class="hover:text-primary"
                                            @click="confirmUrl = '{{ route('dataset.destroy', $item->id) }}', document.querySelector('.confirmModal').classList.remove('hidden') ">
                                        ...
                                        <button class="hover:text-primary"
                                            @click="isEdit = true, selectedValue = { id: '{{ $item->id }}', name: '{{ $item->name }}', description: '{{ $item->description }}',  price: '{{ $item->price }}', is_show: '{{ $item->is_show }}', image_path: '{{ $item->image }}' }, modalCreate = true">
